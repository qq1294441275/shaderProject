...
///逐像素
Shader "myShader/Specular_Pixel" {
	Properties {
	...
	SubShader {
		...
		v2f vert(a2v inPut){
			...
			outPut.worldVertex = mul(inPut.normal,unity_WorldToObject).xyz;
			return outPut;
		}
		...
		fixed4 frag (v2f outPut):SV_Target{
			...
			//当前点到摄像机的方向
			fixed3 viewDir = normalize(_WorldSpaceCameraPos.xyz - outPut.worldVertex);
